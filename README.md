...
- `index.html`: 공유용 페이지입니다. 현재는 `data/profiles.json`을 읽어 프로파일 목록과 복사 버튼을 표시합니다.
- `admin.html`: 관리용 페이지입니다. GitHub Personal Access Token을 입력해 GitHub Contents API로 `data/profiles.json`을 수정 커밋합니다.
- `data/profiles.json`: 현재 초안 데이터 저장 파일입니다.
...
현재 초안은 모든 프로파일 문자열을 `data/profiles.json` 하나에 넣습니다. 하지만 WoW 애드온 export 문자열은 매우 길 수 있습니다.
